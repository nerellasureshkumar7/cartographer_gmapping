...
int main(int argc, char ** argv)
{
  ...
  while (nh.ok())
  {
    ...
    try
    {
      ...
      float yaw_angle = tf::getYaw(pose_stamped.pose.orientation);
      ROS_INFO("%f", yaw_angle);
      ROS_INFO("%f",pose_stamped.pose.position.x );
      ROS_INFO("%f",pose_stamped.pose.position.y );
      if(is_stamped)
      {
        ...
        nh.setParam("/pose_x", pose_stamped.pose.position.x);
        nh.setParam("/pose_y", pose_stamped.pose.position.y);
        nh.setParam("/theta", yaw_angle);
      }
    }
